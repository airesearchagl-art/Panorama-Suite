...
案件パッケージ作成から平面図ピン配置へ、案件データを直接渡せます。

- 受け渡し元: 案件パッケージ作成（Project Packager）
- 受け渡し先: 平面図ピン配置（FloorMap Builder）
- 操作: 案件パッケージ作成の「平面図ピン配置へ送る」を押す
- 保存方式: `sessionStorage`
- キー名: `panorama-suite:handoff:project`
- 外部送信: なし
...
4. 必要に応じて平面図画像を再登録する。
5. 平面図にピンを配置する。
6. 更新済み案件データ（updated-project.json）を書き出す。
7. 案件パッケージ作成で再読み込みし、ZIPとして保存する。
