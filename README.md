...
Panorama Suite は、建築設計・施工管理で扱う360度パノラマ画像を、確認、変換、案件整理、平面図ピン配置、レビュー、共有ZIP作成までつなげるWebポータルです。

画像や案件データはブラウザ内で処理します。外部APIへの送信やクラウドアップロードは行いません。
...
初めて使う場合は、Portalの「はじめての方へ」または [はじめての使い方](https://panorama-suite.vercel.app/tutorial) を開いてください。サンプル案件を読み込むと、画像確認からShare Hubの共有ZIP作成までの流れを画面上で追えます。

推奨フロー:

1. Viewerでパノラマの見え方を確認する
2. パノラマ品質チェックで画像の形式、サイズ、比率、名前を確認する
3. パノラマ画像変換で提出・管理しやすい形式に整える
4. 案件パッケージ作成で案件情報、画像、平面図、品質チェック結果をまとめる
5. 平面図ピン配置で撮影位置を整理する
6. レビュー書き出しで確認用レポートとコメントを整理する
7. Share Hubで共有ZIPを作る

## スクリーンショット

| Portal / Dashboard | Tutorial |
| --- | --- |
| ![Portal Dashboard](public/screenshots/portal-dashboard.png) | ![Tutorial](public/screenshots/tutorial.png) |

| FloorMap | Share Hub |
| --- | --- |
| ![FloorMap](public/screenshots/floormap.png) | ![Share Hub](public/screenshots/share-hub.png) |

## 主な機能

| カテゴリ | ツール | 状態 | 概要 |
| --- | --- | --- | --- |
| Viewer | Archview360 | 外部ツール | 建築パノラマの閲覧・レビュー |
...
| レビュー | レビュー書き出し（Review Exporter） | 基本機能版 | レビュー用HTML、印刷/PDF保存、コメント整理 |
| ドキュメント | サムネイル一覧作成（Thumbnail） | 基本機能版 | サムネイル一覧HTML、CSV書き出し |
| レビュー | 画像比較（Diff） | 基本機能版 | A/B画像の左右比較、スライダー比較、簡易差分 |
| 共有 | 共有パッケージ作成（Share Hub） | 基本機能版 | 共有メモ、manifest、index、共有ZIP作成 |
| ガイド | はじめての使い方（Tutorial） | 利用可能 | サンプル案件で基本フローを確認 |
...
サンプル案件は、実画像なしでPanorama Suiteの流れを確認するためのデモデータです。

- 案件名: Panorama Suite サンプル案件
- 用途: オフィス改修レビュー
- パノラマ数: 3
- 平面図数: 1
- ピン数: 3
...
1. PortalまたはTutorialで「サンプル案件を読み込む」を押す
2. Converter、Packager、FloorMap、Review Exporter、Share Hubを順番に開く
3. 各ページのサンプル案件カードで役割を確認する
4. Share Hubで `共有ZIPを書き出し` を押す

実運用では、手元のパノラマ画像、平面図、案件データファイル、レビューHTMLなどを登録してください。

## 共有ZIP

Share Hubでは、共有用の確認HTML、目録データ、関連ファイルを1つのZIPにまとめます。
...
- `share-manifest.json`: 案件名、共有メモ、件数、同梱ファイル一覧を記録する目録データ
- `share-index.html`: 共有内容をブラウザで確認するためのHTML
- `files/`: 登録した project.json、review-report.html、画像、CSVなど

## ローカル処理

Panorama Suiteは、画像処理、品質チェック、ZIP作成、レビューコメント、共有ZIP作成をブラウザ内で行います。

- 外部APIへ画像を送信しません
- クラウドアップロードを行いません
- サンプル案件もブラウザ内の一時データとして扱います
- 共有ZIPはローカルで作成されます
...
- はじめての使い方: https://panorama-suite.vercel.app/tutorial
...
更新内容は [RELEASE_NOTES.md](./RELEASE_NOTES.md) にまとめています。
...
- 各ページに移動できる
- Share HubでZIPを作れる
- `share-index.html` の意味が分かる
- `share-manifest.json` の意味が分かる
- `npm run build` が成功する
- Viteの500kB超過警告が出ない
- 外部送信しないローカル処理の説明がある
...
v2.0.0は、Panorama SuiteをGitHub / Vercelで共有するための公開版です。Tutorial、サンプル案件、主要ページ、Share Hub、Docs、Help、READMEの導線を実画面で確認し、外部ユーザーがサンプル案件から共有ZIP作成まで進める状態を基準にしています。
...
`/tutorial` で、初めて使うユーザー向けの「はじめての使い方」を確認できます。

目的:

- Panorama Suiteが何をするツールか理解する
- どの順番で使うか理解する
- サンプル案件を通じて、画像確認から共有ZIPまでの流れを確認する

推奨操作フロー:

1. パノラマ品質チェックで画像の形式、サイズ、2:1比率、名前を確認する
2. パノラマ画像変換で画像形式、画質、解像度を整える
3. 案件パッケージ作成で案件情報、画像、平面図、品質チェック結果をまとめる
4. 平面図ピン配置で平面図にパノラマ位置を置く
5. レビュー書き出しで確認用レポートとコメントを整理する
6. 共有パッケージ作成で共有ZIPを作る
...
- 平面図数: 1
- ピン数: 3
- 共有メモ例: レビュー会議前に、品質チェック、平面図ピン、共有ZIPまでの流れを確認します。

サンプル案件はメタ情報のみです。実画像ファイルは含まれません。画像処理を試す場合は、手元の jpg / png / webp を読み込んでください。

初回導線:

- Portalに「はじめての方へ」カードを表示
- 初回のみチュートリアル案内バナーを表示
- 「あとで見る」を押すと `localStorage` に記録し、再表示しない

主要ページには「このページでできること」カードを追加し、初心者でも操作順が分かるようにします。

## v1.7 サンプル案件完走と公開前QA

v1.7では、サンプル案件を読み込んで、各ページの役割を確認し、Share Hubで共有ZIPを書き出すところまで進める導線を強化しています。
...
- サンプルピン
- サンプルレビュー項目
- Share Hubで作成される `share-manifest.json` と `share-index.html`

サンプル案件の使い方:

1. Portalまたは `/tutorial` で「サンプル案件を読み込む」を押す
2. Converterで、サンプルパノラマの一覧と「ここで画像変換を行う」説明を確認する
3. Packagerで、案件構成、パノラマ数、平面図数、ピン数を確認する
4. FloorMapで、プレースホルダー平面図とサンプルピンを確認する
5. Review Exporterで、サンプルレビュー項目を確認する
6. Share Hubで、共有メモ例とサンプルメタ情報を確認する
7. `共有ZIPを書き出し` で `panorama-share-package.zip` を保存する

サンプル案件は実画像なしのデモデータです。実運用では、手元の画像、平面図、案件データファイル、レビューHTMLなどを各ページで登録してください。
...
- 案件情報が整理されている
- 平面図ピンが確認できる
- Review Exporterでレビュー情報を確認できる
- Share HubでZIPを書き出せる
- `share-index.html` をブラウザで開ける
- `share-manifest.json` に案件情報が入っている
- 外部送信しないローカル共有であることを確認する
...
はじめての使い方は `http://localhost:5173/tutorial` です。
